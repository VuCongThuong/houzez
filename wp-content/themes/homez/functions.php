...
define( 'HOMEZ_THEME_VERSION', '1.0.4' );
...
add_action( 'wp-realestate-process-submission-after-save', 'create_product_while_create_property' );
function create_product_while_create_property($post_id)
{
    $post_data = get_post_meta($post_id);
    $post_data = array_map(function ($subArray) {
        return $subArray[0];
    }, $post_data);
	...
	if ($post_data['_property_price'])
	{
		$price = $post_data['_property_price'];
	}

	if ($post_data['_property_price_custom'])
	{
		$price = $post_data['_property_price_custom'];
	}

	if ($post_data['_property_featured_image_img'])
	{
		$image_id = attachment_url_to_postid($post_data['_property_featured_image_img']);
	}
    ...
    if ($post_data && $price) {
		...
		$product->set_name( $post_data['_property_title'] );
		$product->set_regular_price( $price );
		$product->set_image_id($image_id);
		$product->set_sku('Prop_ID: '. $post_id);

		$product->set_short_description($current_user->user_login);
...
function render_custom_order_field($post) {
    ...
    ?>
    <p>
        <label for="custom-order-field">Transfer money to agent:</label>
